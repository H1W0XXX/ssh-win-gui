...
// GNU rsync rejects uncompressed literal tokens larger than 32 KiB with
// "invalid uncompressed token length", so keep sender chunks protocol-sized.
...
func normalizeLocalRequest(localDir, requested string) (string, string) {
	local := localDir
	if local == "/" {
...
func (st *Transfer) SendFileList(localDir string, paths []string, excl *filterRuleList) (*fileList, error) {
	...
	for _, requested := range paths {
		local, requested := normalizeLocalRequest(localDir, requested)
		...
		if st.Opts.DebugGTE(rsyncopts.DEBUG_FLIST, 1) {
			st.Logger.Printf("  fs.Walk(%q, %q), strip=%q", local, requested, strip)
		}
